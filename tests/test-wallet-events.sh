...
expense_params='{ "requestId" : 3, "publicId" : "Z182Z" }'
refund_params=''
...
./wallet-set-low-balance-threshold.sh ${wallet_id} 500
./get-wallet.sh ${wallet_id}

printf "\\nCreando una entrada\\n"
entry=$(create_entry_raw "INCOME" "$wallet_id" 1000 "${income_params}")
printf "$bold%s$normal\\n\\n" "$entry"
...
./create-wallet-adjustment.sh ${wallet_id} -500 "${adjustment_params}"
./create-wallet-adjustment.sh ${wallet_id} -500 "${adjustment_params}"

printf "\\nConfirma una retirada de efectivo\\n"
./create-wallet-adjustment.sh ${wallet_id} 500 "${adjustment_params}"

withdrawal=$(create_entry_raw "WITHDRAWAL" "$wallet_id" 400 "${withdrawal_params}")
printf "$bold%s$normal\\n\\n" "$withdrawal"
withdrawal_id=$(echo ${withdrawal} | jq .entryId)
./confirm-entry.sh ${wallet_id} ${withdrawal_id}
